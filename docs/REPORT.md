...
| Metric | Value |
|--------|-------|
| Judge Type | Keyword-based scoring (Gemini API quota exhausted) |
...
4. **LLM judge agreement** - Near-zero Kappa (0.054) because keyword fallback was used instead of actual LLM scoring

### Key Lessons

1. **Auto-labeled evaluation data produces misleadingly optimistic results.** The previous 70.41% accuracy was inflated by ~24 points because the test labels came from the same system being evaluated. After human verification of all 196 labels, the true accuracy is 46.43%.

2. **Keyword-based scoring cannot match human judgment.** The LLM judge used keyword fallback due to API quota limits, resulting in near-zero agreement with human scores. This demonstrates why actual LLM scoring (or human review) is needed for quality assessment.

Future work should focus on:
1. Human-labeled training data
2. Better escalation rules
3. Embedding-based retrieval
4. LLM-powered reply generation with proper API quota management
